...
\pause
For each $n\in \mathbb{Z}_+$, take $x_n\in A$ with $0 < d_S(x_n,a) < 1/n$ and $d_T(x_n,L) \geq \epsilon$.\\
\pause
...
\pause
Then
$$\lim_{t\rightarrow 0} f(t) = (1,0).$$
\pause
To see this, let $\epsilon > 0$.
\pause
Choose $\delta > 0$ by $\delta = \min\{1,\epsilon/\sqrt{2}\}$.
\pause
Then for $d_{\mathbb{R}}(t,0) < \delta$, we have
\pause
{\small
$$d_{\mathbb{R}^2}(f(t),(1,0)) = \sqrt{(1-(1-t))^2 + (0-t^2)^2} < |t|\sqrt{1+t^2} \leq |t|\sqrt{2} \leq \epsilon$$
}
\pause
