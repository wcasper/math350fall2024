...
Suppose that $$x\geq 0$$ and $$0\leq x\leq \epsilon$$.\\
Since $$x\geq 0$$, so $$x=0$$ or $$x>0$$.\\
Thus it suffices to show that $$x > 0$$  must be false.\\
We will do so by contradiction.\\
Assume $$x > 0$$ is true.  Then $$0 < 1/2$$, so by A8: $$0 < (1/2)x = x/2$$.\\
Therefore if we take $$\epsilon = x/2$$, we find

$$x \leq x/2.$$

Subtracting $$x/2$$ from both sides and using A7: 

$$x/2 \leq 0.$$

However, since $$x/2 > 0$$ this contradicts the trichotomy A6.
Thus we must conclude that $$ x > 0$$ is false.
